...
def test_做法全库是同一套书写风格():
    """431 步是分 10 批写的，**风格必须统一**，否则同一顿饭的两道菜看着像两个人写的。

    2026-09-15 实测到的偏差：第 3/6/7 批每句都以「。」结尾，第 5/10 批用阿拉伯数字
    （"250 克"），其余用中文数字 —— 已统一到多数派（无句号 + 中文数字）。
    这条测试就是那次整理的护栏：以后谁再写进去一个 "300 克" 或句末句号，这里先红。
    """
    bad = []
    for r in DB.recipes:
        for s in r.steps:
            if any(ch.isdigit() for ch in s):
                bad.append(f"{r.name}: 阿拉伯数字「{s}」")
            if s.endswith(("。", "；", ";")):
                bad.append(f"{r.name}: 句末标点「{s}」")
            if " " in s or "　" in s:
                bad.append(f"{r.name}: 夹了空格「{s}」")
    assert not bad, bad[:8]
